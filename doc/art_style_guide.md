...
> **Rivers no longer use any of this (2026-09-09).** Everything below still
> describes ROADS accurately, and the shoreline subsection after it is still
> current. Rivers moved off stub art entirely and are now drawn as curves in
> code — see §10a for what replaced it and why. River paragraphs below are
> kept because the reasoning behind them (rimless fills, overlap at joins,
> matching the coast's blue) carried straight over to the new implementation.
>
> Note also that `tools/make-road-stubs.ps1` and `tools/make-river-stubs.ps1`,
> cited below, **are not in the repo** — only `make-shore-stubs.ps1` survives.
> `assets/roads/*.png` are checked in, so roads are unaffected in practice,
> but there is no script to regenerate them from.
...
## 10a. Rivers — procedural curves, no art at all (2026-09-09)

User report: *"rivers are very square and do not feel natural. They turn at
90 degree angles, do not appear to meander, and are not animated at all."*
All three were true, and only the first was the renderer's fault.

**Why the stub technique could not be fixed in place.** §10's approach stamps
a hub at the exact tile centre plus one centre→edge-midpoint band per
connection. Every path it can express is therefore Manhattan: a turning tile
is two perpendicular axis-aligned bands meeting in a literal elbow. The stub
art's hand-authored wobble cannot help, because §10 requires that wobble to
taper to zero at the centre and at the edge crossing so neighbours line up —
which is exactly where the turn happens. Adding a corner elbow asset would
have fixed corners while leaving straight runs ruler-straight, and there was
no generator script left to author it with.

**What replaced it** (`render.js`, `drawRiverOverlay` and its neighbours):
one quadratic Bézier per tile, through a waypoint jittered ±0.15 tile off the
tile centre. Two properties do the work:

1. Every point is hashed from **tile coordinates**, never from the tile
   object — `drawRememberedTile` is handed a fog snapshot rather than a live
   tile, and both must produce the identical curve. The hash and jitter are
   lifted from `render3d.js`'s `riverHash`/`riverWaypoint` (salts 11 and 13
   included), so the 2D and 3D views finally agree on where a river runs.
2. A direction's endpoint is not the edge midpoint but the point where the
   segment `waypoint(self) → waypoint(neighbour)` **crosses the shared
   boundary**. Because that point lies on the segment, the curve's tangent
   there is collinear with it — and so is the neighbour tile's, since it is
   the same segment. The join is tangent-continuous with no kink, and each
   tile's curve stays inside its own footprint. Verified over every river edge
   on a generated map: 238 edges, zero crossing mismatches, zero
   non-collinear joins, every crossing inside its edge span.

**Softening the banks** (2026-09-09, user-directed: the first pass "read more
like hard blue lines drawn over the terrain"). A plain canvas stroke has a
hard edge, so the band met the ground in a one-pixel blue-to-green cut. Four
translucent stops under the channel — two of shoreline sand, two of shallow
blue — accumulate into a gradient. Being translucent, they blend with whatever
terrain is actually beneath, so the same wash reads blue-green over plains and
blue-tan over desert with the renderer never sampling terrain colour. The sand
tone is sampled from `shore_cardinal.png` deliberately: that stub is how this
game already draws water meeting land at every coastline, and a river should
speak the same language rather than invent a second one.

**Varying the width** (same session, user-directed). The retired PNG's band
ran 7–12px of 64; a constant-width stroke reads as pipe. Each channel is split
into six exact sub-arcs (via the Bézier blossom, so the shape is unchanged)
and each is stroked at a width from a two-sine function of **map** position.
Map position, not tile position and not a per-tile hash, is load-bearing: the
width at a shared crossing must come out the same from both tiles or every
...
- Overlap at joins, don't butt — the channel overshoots slightly past the
  crossing, the same insurance §10's stubs got from overshooting the centre.
- **New:** any pass drawn at alpha < 1 must use `lineCap: "butt"`. At alpha 1
  two tiles' overshooting round caps overlap invisibly (same colour) and
  usefully. Below alpha 1 the same overlap double-composites into a darker
  bead sitting exactly on the tile boundary — the seam §10's taper rule
  existed to prevent, reintroduced by the back door.

**Flow animation.** Rivers were the only water on the map that didn't move —
ocean and coast have animated since they were authored. A faint highlight
slides downstream along each tile's own curve at 0.36 Hz, its alpha a sine
over the whole travel so it fades in and out rather than switching. That
envelope also removes the need for any cross-tile phase alignment, which is
why a scrolling `setLineDash` was rejected: dash phase cannot align across
tiles that each stroke their own path, so every boundary would show a
discontinuity. Direction comes from `tile.riverFlowTo`, set by worldgen;
where it's absent (maps generated before this) the glint is skipped rather
than guessed, because no rule reading one tile's own flags can be coherent —
a river turning north gives a corner `{w,n}` and the tile above `{s,e}`, and
any fixed precedence makes one such pair point at each other.

Gated on `UI.motion.isReduced()` and on tile size, live tiles only. Measured
on a clutter-free river tile: 5% peak luminance swing on band pixels, 0.44%
change per frame at 60fps — far under the project's no-flashing rule.

**Half the squareness was never here.** See `worldgen.js`'s `generateRivers`:
its walk scored neighbours with a six-value terrain-class lookup that tied
across all flat land, and broke ties toward a fixed compass direction. On the
old code the longest dead-straight east–west run averaged 21.7 tiles and hit
38 on a 56-wide map. No renderer could have made that meander.
