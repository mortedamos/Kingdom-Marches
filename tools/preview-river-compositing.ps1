<#
.SYNOPSIS
  Verification-only: composites the river stubs (rotated 0/90/180/270,
  exactly as js/ui/render.js drawRiverOverlay will at runtime) over a
  terrain tile for several hasRiver edge patterns. Not part of the shipped
  asset pipeline.
#>

Add-Type -AssemblyName System.Drawing

$ts = 64
$cardinal = [System.Drawing.Image]::FromFile((Resolve-Path "assets\rivers\river_cardinal.png"))
$hub      = [System.Drawing.Image]::FromFile((Resolve-Path "assets\rivers\river_hub.png"))
$terrain  = [System.Drawing.Image]::FromFile((Resolve-Path "assets\terrain\plains_1.png"))

$cardAng = @{ e = 0; s = 90; w = 180; n = 270 }

function Draw-Stub($g, $img, $angleDeg) {
    $g.TranslateTransform([single]($ts/2), [single]($ts/2))
    $g.RotateTransform([single]$angleDeg)
    $g.DrawImage($img, [single](-$ts/2), [single](-$ts/2), [single]$ts, [single]$ts)
    $g.ResetTransform()
}

function Render-Tile($edges) {
    $bmp = New-Object System.Drawing.Bitmap($ts, $ts, [System.Drawing.Imaging.PixelFormat]::Format32bppArgb)
    $g = [System.Drawing.Graphics]::FromImage($bmp)
    $g.InterpolationMode = [System.Drawing.Drawing2D.InterpolationMode]::NearestNeighbor
    $g.DrawImage($terrain, (New-Object System.Drawing.Rectangle(0,0,$ts,$ts)), 0,0,$ts,$ts, [System.Drawing.GraphicsUnit]::Pixel)
    if ($edges.Count -gt 0) {
        Draw-Stub $g $hub 0
        foreach ($d in "e","s","w","n") { if ($edges -contains $d) { Draw-Stub $g $cardinal $cardAng[$d] } }
    }
    $g.Dispose()
    return $bmp
}

# spring (1 edge), straight through (2 opposite), bend (2 adjacent),
# T (3 edges), all 4
$patterns = @(
    @("e"),
    @("n","s"),
    @("n","e"),
    @("n","s","e"),
    @("n","s","e","w")
)

$cols = $patterns.Count
$scale = 3
$pad = 6
$cellW = $ts * $scale + $pad
$out = New-Object System.Drawing.Bitmap(($cellW * $cols), ($ts * $scale))
$og = [System.Drawing.Graphics]::FromImage($out)
$og.InterpolationMode = [System.Drawing.Drawing2D.InterpolationMode]::NearestNeighbor
$og.Clear([System.Drawing.Color]::FromArgb(255, 40, 40, 40))
for ($i = 0; $i -lt $cols; $i++) {
    $tile = Render-Tile $patterns[$i]
    $og.DrawImage($tile, [single]($i * $cellW), 0, [single]($ts * $scale), [single]($ts * $scale))
    $tile.Dispose()
}
$og.Dispose()
$out.Save((Join-Path (Get-Location) "assets\img\working\river_compositing_preview.png"))
$out.Dispose()
$cardinal.Dispose(); $hub.Dispose(); $terrain.Dispose()
Write-Host "Saved assets\img\working\river_compositing_preview.png"
